...
ST7796_t3 *createDisplay(const ST7796_Config &config)
{
  if (config.cs >= 0)
  {
  ...
  return new ST7796_t3(config.cs, config.dc, config.mosi, config.sck, config.rst);
}

ST7796_Display::ST7796_Display(const ST7796_Config &config) : display(createDisplay(config)), asyncUpdates(config.asyncUpdates)
{
  static size_t displayNum{};
  Serial.print(F("Init ST7796 display #"));
  ...
  if (config.cs < 0)
  {
    // Try to handle the ST7796 displays without CS pins
...
void ST7796_Display::drawPixel(int16_t x, int16_t y, uint16_t color565)
{
  display->drawPixel(x, y, color565);
}

void ST7796_Display::drawFastVLine(int16_t x, int16_t y, int16_t height, uint16_t color565)
{
  display->drawFastVLine(x, y, height, color565);
}

void ST7796_Display::drawText(int16_t x, int16_t y, char *text)
{
...
bool ST7796_Display::isAvailable() const
{
